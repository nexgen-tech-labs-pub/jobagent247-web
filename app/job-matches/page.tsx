...
import {
  ...
  Search, CheckCircle, XCircle, Zap, Loader2, ChevronDown,
  ChevronUp, RefreshCw, ChevronLeft, ChevronRight,
} from 'lucide-react'
...
function Pagination({ page, total, onPage }: { page: number; total: number; onPage: (p: number) => void }) {
  ...
  return (
    <div className="flex items-center justify-between mt-6" style={{ color: '#64748B' }}>
      <p className="text-sm">
        Page {page} of {totalPages} <span className="opacity-60">({total} jobs)</span>
      </p>
...
export default function JobMatchesPage() {
  ...
  const fetchJobs = useCallback(async (kw: string, tf: string, pg: number) => {
  ...
  const loadAndScore = useCallback(async (kw = '', tf = 'All', pg = 1) => {
    setLoading(true)
    const { jobs: jobList, total: t } = await fetchJobs(kw, tf, pg)
    setJobs(jobList)
    setTotal(t)
    setPage(pg)
    setLoading(false)
  ...
  const startScrape = async () => {
    ...
    const res = await fetch('/api/jobs/scrape', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        keywords: keyword ? keyword.split(' ') : ['developer'],
        location: 'London',
        sites: [],
      }),
    })
    ...
    pollRef.current = setInterval(async () => {
      ...
      if (!poll.ok) return
      const { status, error } = await poll.json() as { status: ScrapeStatus; results_count: number; error: string | null }

      setScrapeStatus(status)
      if (status === 'done') {
        stopPolling()
        await loadAndScore(keywordRef.current, typeRef.current, 1)
  ...
  const handleFilter = () => loadAndScore(keyword, typeFilter, 1)
  const handlePage = (p: number) => loadAndScore(keywordRef.current, typeRef.current, p)
      ...
      <GlassCard className="p-4 mb-6">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          {(['All', 'Permanent', 'Contract'] as const).map(t => (
            <button key={t} onClick={() => { setTypeFilter(t); loadAndScore(keywordRef.current, t, 1) }}
                ...
                <GlassCard key={job.id} hover className="p-6">
                  <div className="flex flex-col md:flex-row md:items-start gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      {brief && (
                        <p className="text-sm leading-relaxed line-clamp-2 mb-3" style={{ color: '#94A3B8' }}>{brief}</p>
                      )}
                      {analysis && (
                        <button
