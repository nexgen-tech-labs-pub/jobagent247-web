...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { allowed, remaining, code, lockedUntil } = await checkQuota(supabase, user.id, plan, 'job_fit', locale)
    if (!allowed) {
      return NextResponse.json({ error: 'Free plan allows 2 job fit analyses. Upgrade for more.', remaining: 0, code, lockedUntil }, { status: 429 })
    }
