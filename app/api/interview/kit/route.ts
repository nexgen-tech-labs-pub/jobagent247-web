...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { allowed, remaining, code, lockedUntil } = await checkQuota(supabase, user.id, plan, 'interview_kit', locale)
    if (!allowed) {
      return NextResponse.json({ error: 'Free plan allows 1 interview kit. Upgrade for more.', remaining: 0, code, lockedUntil }, { status: 429 })
    }
